...
/mob/living/simple_animal/hostile/abnormality/seasons/proc/SpringMelee(mob/living/attacked_target)
	...
	to_chat(world, "turfx = [turfx], turfy = [turfy], attack_orientation = [attack_orientation],orientation = [orientation]")
	can_act = TRUE
...
/mob/living/simple_animal/hostile/abnormality/seasons/proc/SpringMeleeHit(turf/T)
	...
	for(var/mob/living/L in T)
		if(faction_check_mob(L))
			continue
		L.deal_damage(melee_damage_upper * 1.5, melee_damage_type)
		been_hit = TRUE
	if(been_hit)
...
/mob/living/simple_animal/hostile/abnormality/seasons/proc/SpringSpecial(mob/living/attacked_target)
	special_attack_cooldown = world.time + special_attack_cooldown_time
	to_chat(world, "Did something - special attack")
	can_act = FALSE
	can_act = TRUE
