	...
	for(var/x = 1 to 3)
		...
		for(var/i = 1 to 8)
			...
			for(var/turf/T in hit_turfs)
				...
				//for(var/mob/living/L in been_hit) DELME TODO - Fix
				//	if(ishuman(L))
				//		L.apply_void(4)
				new /obj/effect/temp_visual/kinetic_blast(T)
				if(prob(10))
...
/datum/status_effect/stacking/void/proc/DoGrab(client/C, num, screen_location = "Center,Center")
	...
	for(var/i = 1 to num)
		...
		T.icon_w = rand(-160,160)
		T.icon_z = rand(-160,160)
		T.layer = FLOAT_LAYER
		T.plane = HUD_PLANE
		T.appearance_flags = APPEARANCE_UI_IGNORE_ALPHA
		T.screen_loc = screen_location
...
/datum/status_effect/stacking/void/process()
	...
	if(!grabbing)
		return
	listclearnulls(overlay_objects)
	...
	if(!owner)
		qdel(src)
		return
//	owner.Immobilize(1) DELME - does nothing for now. TODO - Fix.
